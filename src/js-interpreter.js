...
export default class {
  ...
  displayNode() {
    ...
    if (
      this.renderData.inputs.length !== 0 &&
      this.renderData.inputs[0].type !== "button"
    ) {
    ...
    if (
      supportsFileApi &&
      this.ODCore.currentNode === this.ODCore.tree.header.start_node
    ) {
      document
        .getElementById(selectedDiv)
        .querySelector("#files")
        .addEventListener("change", loadSaveData, false);
    }
